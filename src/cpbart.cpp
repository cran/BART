    ...
    if(iy[k]==0) iz[k]= -rtnorm(-bm.f(k), binaryOffset, 1., gen);
      ...
      if(i>=burn) {
	   ...
	   Rcpp::List lists(m*treesaslists);
	      ...
	      treess << bm.gettree(jj);

	      #ifndef NoRcpp
	      varcount.row(treedrawscnt)=varcount.row(treedrawscnt)+bm.gettree(jj).tree2count(p);
	      if(treesaslists) lists(jj)=bm.gettree(jj).tree2list(xi, 0., 1.);
	      #endif
	    }
	    #ifndef NoRcpp
