...
## ---- include=TRUE, echo=TRUE,dependson=paste0("s1-",1:2),collapse=TRUE----
names(bf)
length(bf$sigma)
length(bf$yhat.train.mean)
dim(bf$yhat.train)

## ---- include=TRUE, echo=TRUE,dependson="s1-2", out.width='50%',fig.align='center'----
plot(bf$sigma)
abline(v=burn,lwd=2,col="red")

## ---- include=TRUE, echo=TRUE,out.width='70%',fig.align='center',dependson=paste0("s1-",1:2)----
...
## ---- include=TRUE, echo=TRUE,out.width='70%',fig.align='center',dependson="s1-2"----
...
## ----pred1,include=TRUE, echo=TRUE,cache=TRUE,message=FALSE,results="hide",dependson="ttsplit"----
set.seed(99)
bfp1 = wbart(xtrain,ytrain,xtest) #predict.wbart wants a matrix

## ---- include=TRUE, echo=TRUE,dependson="pred1",collapse=TRUE------------
dim(bfp1$yhat.test)
length(bfp1$yhat.test.mean)

## ----pred2,include=TRUE, echo=TRUE,cache=TRUE,message=FALSE,results="hide",dependson="ttsplit"----
set.seed(99)
bfp2 = wbart(xtrain,ytrain)
yhat = predict(bfp2,as.matrix(xtest)) #predict wants a matrix

## ---- include=TRUE, echo=TRUE,dependson="pred2",collapse=TRUE------------
dim(yhat)
summary(as.double(yhat-bfp1$yhat.test))

## ----bfthin,include=TRUE, echo=TRUE,cache=TRUE,message=FALSE,results="hide",dependson="ttsplit"----
...
## ---- include=TRUE, echo=TRUE,collapse=TRUE------------------------------
dim(bfthin$yhat.train)
dim(yhatthin)

## ---- include=TRUE, echo=TRUE,out.width='60%',fig.align='center',dependson=c("ttsplit","bfthin","pred1")----
