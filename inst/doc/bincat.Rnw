...
values 0 or 1.  Given these $y_i$, we introduce the auxiliary Normal
latents, $z_i$, as the outcome for a continuous BART with unit
variance where $i$ indexes subject and $\Phi$ is the standard Normal
...
$p_0=\Phi(\mu_0)$.  If $\mu_0=0$, which is the default, then the $p_i$
...
(which is the default).  The \code{pbart} (\code{mc.pbart}) function
...
assumption can be relaxed by assuming the latents come from the
...
The outcome, \code{y.train}, is provided as an integer with values 0 or 1.
Logistic BART is provided by the \code{lbart} function.  Unlike probit
BART where we sample Normal latents, $z_i$, with a fixed variance
$\sd^2=1$; with Logisitic BART, we sample Normal latents, $z_i$, with
a random variance $\sd_i^2$.  If
${\sd^2_i} = 4\psi_i^2 \where \psi_i ~ \mbox{Kolmogorov-Smirnov}$,
then the $z_i$ follow the Logistic distribution.  Sampling $\psi_i$
from the Kolmogorov-Smirnov distribution is described by
\cite{Devr86}.  So, the conditionally Normal latents, $z_i|\sd_i^2$,
are the outcomes for a continuous BART with a known heteroskedastic
variance, $\sd_i^2$.  Since Logistic latents are more flexible, there
is no centering parameter, i.e., $\mu_0=0$.  Therefore, the
probabilities are $p_i=F(f(\bm{x}_i))$ where $F$ is the standard
...
$\alpha$ parameter.
\begin{align*}
\wrap{s_1, \., s_P} & \prior \Dir{\alpha/P, \., \alpha/P} \\
\frac{\alpha}{\alpha+\rho} & \prior \Beta{a}{b} \\
\end{align*}
